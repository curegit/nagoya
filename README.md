...
このテーマでは、ツリーの葉ページ（単一ファイルページまたはリーフバンドル）を通常の記事として扱います。
...
- search と archive は、葉ページ（リーフバンドルを含む）のみを対象とし、さらに固定ページは除外されます
...
現在の制約として、複数ページ間で循環参照するように `ref` を記述するとビルドがデッドロックします。
...
| `authorLink`           | String             | 著者ページまたはメールアドレスのハイパーリンクを設定します（上記 `author` 設定が必要）                                                                          |
...
| `alwaysIncludeLastmod` | Boolean            | 作成日と最終更新日が同一の場合でも最終更新日を表示するべきかを設定します                                                                                        |
...
| `showSectionInList`    | Boolean            | リストページに各記事の所属セクションを表示するか否かを設定します                                                                                                |
| `showTagsInList`       | Boolean            | リストページに各記事のタグリストを表示するか否かを設定します                                                                                                    |
...
| `relatedNum`           | Integer            | 各記事下部に表示する記事リストにおける関連記事の最大数を設定します                                                                                              |
| `randomNum`            | Integer            | 各記事下部に表示する記事リストにおけるランダム記事の数を設定します                                                                                              |
| `randomNumFillTo`      | Integer or Boolean | 各記事下部に表示する記事リストがこの数になるまでランダム記事で埋めます（`false` を設定すると無効、`true` を設定すると `relatedNum` + `randomNum` まで埋めます） |
...
| `showTagsAboveArticle` | Boolean            | 各記事上部にタグリストを表示するか否かを設定します                                                                                                              |
...
| `authorLink`      | String  | 著者ページへのハイパーリンク、または著者メールアドレスを表すリンクを設定します（[ページパラメータ](#ページパラメータ)か[サイトパラメータ](#サイトパラメータ)の `author` 設定が必要） |
| `lang`            | String  | このページの言語を明示し、サイト全体の言語設定 `site.Language.Locale` に対して上書きします                                                                                           |
| `pin`             | Boolean | このページへのリンクをサイドバーにピン留めします（葉ページ、リーフバンドル専用）                                                                                                     |
| `page`            | Boolean | このページを記事ではなく、固定ページとして扱います（葉ページ、リーフバンドル専用）                                                                                                   |
...
- トップページやリストページの内容の始まり部分は、ブランチバンドル `_index.md` で制御できます。
